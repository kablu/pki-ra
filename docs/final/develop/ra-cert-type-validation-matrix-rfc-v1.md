...
| S4 | Wildcard uses DNS method | A wildcard is validated by the DNS method only, never the HTTP file method. **WLCA internal (CA not publicly exposed):** achieve the same via AD instead — confirm the whole domain namespace is on the requesting org's AD approved-domain list | HTTP proves one host; DNS proves the whole namespace, matching the wildcard's scope (BR 3.2.2.4). The AD approved-domain list gives the same namespace-level assurance for internal, non-public certs |
